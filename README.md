...
│   │   ├── scheme_tool.py            (Subsidy policy database engine)
...
Run the Python application or load the modular workspace:
python
...
Open krishi_mitra_kaggle.ipynb in Kaggle.
In the right-hand panel (Settings), toggle Internet ON (required for vision downloads, weather queries, and mandi API connections).
In the top menu, go to Add-ons -> Secrets and add the following keys:
